...
//		double ddSelfTime = runDoubleDoubleSelf(10000000);
		...
		System.out.println("DoubleDouble VS double performance factor = " + ddTime/doubleTime);
//		System.out.println("DoubleDoubleSelf VS double performance factor = " + ddSelfTime/doubleTime);
		
		System.out.println("DoubleDouble VS BigDouble performance factor = " + ddTime/bigDecTime);
	}
...
//			System.out.println(det);
	...
	public double runDoubleDouble(int nIter)
	{
		...
		for (int i = 0; i < nIter; i++) {
			...
			DoubleDouble det = a.multiply(a)
					.subtract(aMul.multiply(aDiv));
//			System.out.println(aDiv);
//			System.out.println(det);
		}
		sw.stop();
		System.out.println("DoubleDouble:    nIter = " + nIter 
				+ "   time = " + sw.getTimeString());
		return sw.getTime() / (double) nIter;
	}
	...
	public double runDoubleDoubleSelf(int nIter)
	{
		...
		for (int i = 0; i < nIter; i++) {
			...
			DoubleDouble c = new DoubleDouble(9.0);
			c.selfMultiply(factor);
			DoubleDouble b = new DoubleDouble(9.0);
			b.selfDivide(factor);
			
			DoubleDouble a2 = new DoubleDouble(a);
			a2.selfMultiply(a);
			DoubleDouble b2 = new DoubleDouble(b);
			b2.selfMultiply(c);
			a2.selfDivide(b2);
			DoubleDouble det = a2;
//			System.out.println(aDiv);
//			System.out.println(det);
		...
		sw.stop();
		System.out.println("DoubleDoubleSelf:nIter = " + nIter 
				+ "   time = " + sw.getTimeString());
		return sw.getTime() / (double) nIter;
	}
	*/
